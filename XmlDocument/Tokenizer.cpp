...
int main(int argc, char* argv[])
{
	...
	for (int i = 1; i<argc; ++i)
	{
		...
		try
		{
			Toker t; t.setMode(Toker::xml);        // comment out to show tokenizing for code t.setSingleCharTokens("<>");  // will return same results as above for XML
			if (!t.attach(argv[i]))
			{
			...
			{
				temp = t.getTok();
				std::cout << "  ln: " << t.lines() << ", br lev: " << t.braceLevel() << ", tok size: " << std::setw(3) << temp.length() << " -- ";
				if (temp != "\n")
