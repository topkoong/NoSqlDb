...
*   - createDateTimeElement					: create a DateTime DbElement
*   - createDbCoreElement					: creates a DbCore DbElement
...
*   - createStringUtilitiesElement			: creates a StringUtilities DbElement
*   - createTestUtilities                   : creates a TestUtilities DbElement
*   - createXmlDocumentElement				: creates an XmlDocument DbElement
*   - createXmlUtilitiesElement				: creates an XmlUtilities DbElement
*   - addChildrenToAllElements				: add a child DbElement to parent dbElement with key
...
*   - testQueryonPayLoad					: demonstrate requirement #9
*   - testCreateProjectPackageStructure		: demonstrate requirement #11a
*   - testCreateNewDbFromProjectFile		: demonstrate requirement #11b
*
...
DbElement<PayLoad> createDbCoreElement()
{
	...
	PayLoad pL2;
	pL2.filePath("../DbCore");
	pL2.categories().push_back("DbCore");
...
DbElement<PayLoad> createExecutiveElement()
{
	...
	pL3.categories().push_back("Executive");
...
DbElement<PayLoad> createPersistElement()
{
	...
	pL4.categories().push_back("Persist");
...
DbElement<PayLoad> createQueryElement()
{
	...
	pL5.categories().push_back("Queries");
...
DbElement<PayLoad> createStringUtilitiesElement()
{
	...
	pL6.categories().push_back("Utilities");
...
DbElement<PayLoad> createTestUtilities()
{
	...
	pL8.categories().push_back("Utilities");
...
DbElement<PayLoad> createXmlDocumentElement()
{
	...
	pL9.categories().push_back("XmlDocument");
...
DbElement<PayLoad> createXmlUtilitiesElement()
{
	...
	pL10.categories().push_back("Utilities");
...
bool testR5b() {
	...
	DbProvider dbp;
	DbCore<PayLoad> db = dbp.db();
	if (db.deleteChild("Salman", "Prashar")) {
		dbp.db() = db;
		putLine();
		std::cout << "\n  showing all the database elements:";
		showDb(db);
...
{
	Utilities::title("Demonstrating Requirement #9 - Query on Payload including categories and FilePath");
	putLine();
	DbCore<PayLoad> db;
	DbProvider dbp;
	db = dbp.db();
	//std::cout << "\n\nQuery command: q9.selectcategories(\".*T.*\");\n";
	//q9.selectcategories(".*T.*");
	//putLine(2);
	...
	q9.select(hasCategory).show();

	std::string value = "Test Payload #1";
	auto hasValue = [&value](DbElement<PayLoad>& elem) {
		return (elem.payLoad()).filePath() == value;
	};
	Utilities::putline();

	std::cout << "\n  select on payload value \"" << value << "\"\n";
	q9.from(db.keys());
	q9.select(hasValue).show();
	return true;
}

//----< demonstrate requirement #11a>----------------------------

bool testCreateProjectPackageStructure()
{
	Utilities::title("Demonstrating Requirement #11a - an XML file, that describe my project's package structure and dependency relationships that can be loaded when my project is graded.\n\n");
...
//----< demonstrate second part of requirement #11>----------------------------

bool testCreateNewDbFromProjectFile()
{
	Utilities::title("Demonstrating Requirement #11b - creating a new db from persisted ProjectPackageStructure.xml");
	putLine();
	std::cout << "\nThe database can be restored or augmented from an existing XML file\n";

	DbCore<PayLoad> newDb;
	Persist<PayLoad> persist2(newDb);
	persist2.restoreFromFile("ProjectPackageStructure.xml");
	std::cout << "\n  showing all the database elements:";
	showDb(newDb);
	putLine();
